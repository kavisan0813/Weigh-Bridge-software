...
export function WeightDisplay({
  ...
  return (
    ...
    >
      <div
        className={`text-xs font-bold uppercase tracking-wider ${labelColor} flex items-center justify-between`}
      >
        <span>{label}</span>
        {isNet && (
      ...
      <div className="mt-1 flex items-baseline gap-1">
        <span
          className={`tabular-nums tracking-tight font-sans ${valueTextSize} ${valueColor}`}
        >
          {formattedValue}
        </span>
        {formattedValue !== "—" && (
      ...
      {subtext && (
        <span
          className={`text-[11px] mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}
        >
          {subtext}
        </span>
      )}
