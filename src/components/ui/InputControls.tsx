...
export interface InputProps
  extends InputHTMLAttributes<HTMLInputElement>, BaseInputProps {}
...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const bgColors = darkMode
      ? "bg-[#111827] text-gray-100 placeholder-gray-500"
    ...
    return (
      <div className="flex flex-col gap-1 w-full">
        ...
        <div className="relative flex items-center w-full">
          {leftIcon && (
            <div
              className={`absolute left-3 pointer-events-none ${darkMode ? "text-gray-400" : "text-gray-500"}`}
            >
              {leftIcon}
            </div>
          )}
          ...
          {rightIcon && (
            <div
              className={`absolute right-3 pointer-events-none ${darkMode ? "text-gray-400" : "text-gray-500"}`}
            >
              {rightIcon}
            </div>
          )}
        ...
        {error && (
          <span className="text-xs text-wb-error font-medium">{error}</span>
        )}
        {helperText && !error && (
          <span
...
export interface SelectProps
  extends SelectHTMLAttributes<HTMLSelectElement>, BaseInputProps {}
