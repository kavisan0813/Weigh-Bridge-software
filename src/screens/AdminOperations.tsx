...
function Shell({ children, darkMode: dm, onToggleDark, onLogout, onNavigate, view }: AdminOperationsProps & { children: ReactNode }) {
  return (
    <AppShell
      activeView={view}
      userRole="admin"
...
function Monitoring({ darkMode: dm, onToggleDark, onLogout, onNavigate }: Omit<AdminOperationsProps, "view">) {
  ...
  return (
    <Shell view="monitoring" darkMode={dm} onToggleDark={onToggleDark} onLogout={onLogout} onNavigate={onNavigate}>
...
function Transactions({ darkMode: dm, onToggleDark, onLogout, onNavigate }: Omit<AdminOperationsProps, "view">) { const p = usePalette(dm); const [selected, setSelected] = useState<string | null>(null); const [query, setQuery] = useState(""); const rows = useMemo(() => transactions.filter(r => `${r[0]} ${r[2]}`.toLowerCase().includes(query.toLowerCase())), [query]); const filters = ["Date range", "Weighbridge", "Operator", "Customer", "Material", "Status"]; return <Shell view="transactions" darkMode={dm} onToggleDark={onToggleDark} onLogout={onLogout} onNavigate={onNavigate}><main className="flex-1 overflow-y-auto p-5 md:p-8" style={{ color: p.text }}><PageHeading title="Transactions" subtitle="View and manage all weighment transactions across the 5 weighbridges." action={<button className="rounded-lg px-4 py-2.5 text-xs font-semibold text-white" style={{ background: "#F97316", border: 0, cursor: "pointer" }}>↓&nbsp; Export</button>} />
