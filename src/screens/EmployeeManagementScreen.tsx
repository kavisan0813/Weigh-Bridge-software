...
const FORM_FIELDS = [
  ...
  {
    label: "Additional Weighbridges",
    type: "select",
    ph: "Select additional (optional)",
  },
  { label: "Status", type: "select", ph: "Active" },
];

export default function EmployeeManagementScreen({ darkMode: dm }: Props) {
  const c = pal(dm);
    ...
    >
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <button
          ...
          className="h-11 rounded-lg px-5 text-xs font-bold text-white"
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-4">
        {[
          ...
          <div
            ...
            style={{ background: c.s, border: `1px solid ${c.border}` }}
          >
            <div
      ...
      <div
        className="mb-5 grid gap-3 rounded-xl p-4 md:grid-cols-[minmax(220px,1fr)_repeat(3,180px)]"
        style={{ background: c.s, border: `1px solid ${c.border}` }}
      >
        <input
          value={query}
        ...
        {["Role", "Weighbridge", "Status"].map((x) => (
          <button
            key={x}
            className="flex h-10 items-center justify-between rounded-lg px-3 text-xs"
            style={{
              background: c.input,
              color: c.sec,
              border: `1px solid ${c.border}`,
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-225 text-left">
            <thead
              ...
              style={{ background: c.sub, color: c.muted }}
            >
              <tr>
                {[
                  "Employee",
                  ...
                  <th
                    key={x}
                    className="whitespace-nowrap px-4 py-3.5 text-[11px] font-semibold"
                  >
                    {x}
                  </th>
                ))}
            ...
            <tbody>
              {filtered.map((emp) => (
                ...
                >
                  ...
                  <td
                    className="px-4 py-4 text-[12px] tabular-nums"
                    style={{ color: c.sec }}
                  >
                    {emp.id}
                  </td>
                  <td className="px-4 py-4">
                    <span
                      ...
                      style={{
                        background: dm ? "rgba(251, 146, 60, 0.15)" : "#FFF7ED",
                        color: dm ? "#FB923C" : "#F97316",
                      }}
                    >
                      Operator
                    </span>
                  ...
                  <td
                    className="px-4 py-4 text-[12px] font-semibold tabular-nums"
                    style={{ color: c.text }}
                  >
                    {emp.primary}
                  </td>
      ...
      {open && (
        ...
        >
          <div
            className="w-full max-w-145 overflow-y-auto rounded-xl p-6"
            style={{
              background: c.s,
              border: `1px solid ${c.border}`,
            ...
            <div className="mb-5 flex items-start justify-between">
              <div>
                ...
                <p className="mt-1 text-[12px]" style={{ color: c.muted }}>
                  Create a new operator account and assign weighbridges.
                </p>
              </div>
              <button
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {FORM_FIELDS.map((f) => (
                <label key={f.label} className={f.span ? "sm:col-span-2" : ""}>
                  ...
                  {f.type === "select" ? (
                    <button
                      className="flex h-10 w-full items-center justify-between rounded-lg px-3 text-left text-xs"
                      style={{
                        background: c.input,
                        color: c.muted,
                        border: `1px solid ${c.border}`,
