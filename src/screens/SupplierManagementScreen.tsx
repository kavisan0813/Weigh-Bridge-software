...
const INITIAL_SUPPLIERS: SupplierRow[] = [
  { id: "1", name: "Southern Rocks Co", code: "SUP-00124", type: "Company", contact: "Ravi Kumar", phone: "+91 98400 12345", materialsCount: 6, vehiclesCount: 17, lastWeighment: "19 Aug 2026 · 10:45 AM", status: "ACTIVE" },
  { id: "2", name: "Green Valley Materials", code: "SUP-00125", type: "Contractor", contact: "Suresh Kumar", phone: "+91 98400 23456", materialsCount: 4, vehiclesCount: 12, lastWeighment: "19 Aug 2026 · 09:30 AM", status: "ACTIVE" },
  { id: "3", name: "Chennai Aggregates Pvt Ltd", code: "SUP-00126", type: "Manufacturer", contact: "Priya Kumar", phone: "+91 98400 34567", materialsCount: 8, vehiclesCount: 21, lastWeighment: "18 Aug 2026 · 04:12 PM", status: "ACTIVE" },
  { id: "4", name: "Metro Stone Suppliers", code: "SUP-00127", type: "Distributor", contact: "Arun Kumar", phone: "+91 98400 45678", materialsCount: 2, vehiclesCount: 5, lastWeighment: "15 Aug 2026 · 01:30 PM", status: "INACTIVE" },
  { id: "5", name: "Deccan Quarry Works", code: "SUP-00128", type: "Company", contact: "Rajesh Sharma", phone: "+91 98400 56789", materialsCount: 5, vehiclesCount: 14, lastWeighment: "19 Aug 2026 · 11:20 AM", status: "ACTIVE" },
  { id: "6", name: "Kaveri Mining Corp", code: "SUP-00129", type: "Manufacturer", contact: "Venkatesh Rao", phone: "+91 98400 67890", materialsCount: 7, vehiclesCount: 19, lastWeighment: "17 Aug 2026 · 02:45 PM", status: "SUSPENDED" },
  { id: "7", name: "Apex Sand & Gravel", code: "SUP-00130", type: "Distributor", contact: "Karthik Subramanian", phone: "+91 98400 78901", materialsCount: 3, vehiclesCount: 9, lastWeighment: "14 Aug 2026 · 10:15 AM", status: "BLOCKED" },
];

export default function SupplierManagementScreen({ darkMode: dm, onToggleDark, onLogout, onNavigate }: Props) {
  ...
  const hasActiveFilters = search !== "" || statusFilter !== "ALL" || typeFilter !== "ALL" || wbFilter !== "ALL";
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: bg, color: primaryText, fontFamily: "'Inter', -apple-system, sans-serif" }}>
        ...
        <aside style={{ width: 248, minWidth: 248, height: "100vh", position: "sticky", top: 0, display: "flex", flexDirection: "column", background: dm ? "#1F2937" : "#0F2438", borderRight: `1px solid ${border}`, flexShrink: 0, zIndex: 40 }}>
          <div style={{ padding: "18px 20px", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <div style={{ fontSize: 13, fontWeight: 800, color: "#FFFFFF", letterSpacing: "-0.01em" }}>WEIGHBRIDGE</div>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <header style={{ background: dm ? "#1F2937" : "#0F172A", borderBottom: `1px solid ${border}`, padding: "8px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0, zIndex: 30 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <span style={{ fontSize: 11, fontWeight: 800, color: secondaryGold, letterSpacing: "0.08em" }}>SCREEN 35</span>
              <span style={{ color: "#475569" }}>|</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#F9FAFB" }}>SUPPLIER LIST</span>
            </div>
          ...
          <div style={{ flex: 1, maxWidth: 1440, width: "100%", margin: "0 auto", background: surface, display: "flex", flexDirection: "column", minHeight: "calc(100vh - 49px)" }}>
            ...
            <header style={{ height: 68, padding: "0 32px", background: surface, borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
              <div>
                ...
                <p style={{ fontSize: 14, color: mutedText, margin: "2px 0 0 0" }}>
                  Manage suppliers, company information, materials and weighment activity.
                </p>
              </div>
            ...
            <div style={{ flex: 1, padding: "24px 32px 48px", display: "flex", flexDirection: "column", gap: 24, overflowY: "auto" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 16 }}>
                {[
                  { label: "TOTAL SUPPLIERS", val: "64", support: "Registered suppliers", color: primaryOrange },
                  { label: "ACTIVE", val: "57", support: "Currently active", color: statusSuccess },
                  { label: "WITH WEIGHMENTS", val: "49", support: "Suppliers with transactions", color: secondaryGold },
                  { label: "INACTIVE", val: "7", support: "Requires attention", color: mutedText },
                ].map((kpi, idx) => (
              ...
              <div style={{ background: surface, borderRadius: 12, border: `1px solid ${border}`, padding: 20, display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {hasActiveFilters && (
                  <div style={{ display: "flex", gap: 8, alignItems: "center", paddingTop: 8, borderTop: `1px solid ${divider}` }}>
                    ...
                    {typeFilter !== "ALL" && <span style={chipStyle}>{`Type: ${typeFilter}`}</span>}
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
                  <thead>
                    <tr style={{ background: elevated, borderBottom: `1px solid ${border}` }}>
                      <th style={thStyle} onClick={() => handleSort("code")}>SUPPLIER ID {sortField === "code" ? (sortOrder === "asc" ? "↑" : "↓") : ""}</th>
                      <th style={thStyle} onClick={() => handleSort("name")}>SUPPLIER NAME {sortField === "name" ? (sortOrder === "asc" ? "↑" : "↓") : ""}</th>
                    ...
                    {filteredList.length === 0 ? (
                      <tr>
                        <td colSpan={10} style={{ padding: 48, textAlign: "center", color: mutedText }}>
                          ...
                          <div style={{ fontSize: 15, fontWeight: 800, color: primaryText }}>No Suppliers Found</div>
                          <div style={{ fontSize: 12, marginTop: 4 }}>No suppliers match your search query or filter selection.</div>
                        </td>
                      </tr>
                    ) : (
                      filteredList.map((row) => {
                        const isMenuOpen = activeMenuId === row.id;
                        const stColor = row.status === "ACTIVE" ? statusSuccess : row.status === "SUSPENDED" ? statusWarning : row.status === "BLOCKED" ? statusError : mutedText;
                        const stBg = row.status === "ACTIVE" ? (dm ? "rgba(22,163,74,0.15)" : "#F0FDF4") : row.status === "SUSPENDED" ? (dm ? "rgba(245,158,11,0.15)" : "#FFFBEB") : (dm ? "rgba(220,38,38,0.15)" : "#FEF2F2");
                        ...
                        return (
                          ...
                          >
                            ...
                            {/* Contact Person */}
                            <td style={{ ...tdStyle, fontSize: 12.5, color: primaryText, fontWeight: 600 }}>
                              {row.contact}
                            </td>

                            {/* Phone */}
                            <td style={{ ...tdStyle, fontSize: 12, color: secondaryText, fontFamily: "monospace" }}>
                              {row.phone}
                            </td>
        ...
        <div style={{ display: "flex", justifyContent: "center", padding: "16px 0 40px" }}>
          <div style={{ width: 390, minHeight: 844, background: surface, borderRadius: 24, border: `1px solid ${border}`, boxShadow: "0 20px 40px rgba(0,0,0,0.25)", overflow: "hidden", display: "flex", flexDirection: "column", position: "relative" }}>
            <div style={{ padding: "14px 18px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span style={{ fontSize: 16, fontWeight: 800, color: primaryText }}>Suppliers</span>
              {role === "admin" && (
                <button onClick={() => onNavigate("supplier-add")} style={{ padding: "6px 12px", borderRadius: 6, background: primaryOrange, color: "#FFF", border: "none", fontSize: 12, fontWeight: 800, cursor: "pointer" }}>+ Add</button>
              )}
            </div>
            ...
            <div style={{ flex: 1, overflowY: "auto", padding: "16px 16px 80px", display: "flex", flexDirection: "column", gap: 12 }}>
              <input
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                style={{ width: "100%", height: 42, padding: "0 12px", borderRadius: 8, background: inputBg, color: primaryText, border: `1px solid ${border}`, fontSize: 13, outline: "none" }}
              />

              {filteredList.map((row) => (
                <div key={row.id} onClick={() => onNavigate("supplier-detail")} style={{ padding: 14, borderRadius: 12, background: elevated, border: `1px solid ${border}`, display: "flex", flexDirection: "column", gap: 6, cursor: "pointer" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, fontFamily: "monospace", color: primaryOrange, fontWeight: 800 }}>{row.code}</span>
                    <span style={{ padding: "2px 8px", borderRadius: 999, fontSize: 10, fontWeight: 800, background: row.status === "ACTIVE" ? (dm ? "rgba(22,163,74,0.15)" : "#F0FDF4") : elevated, color: row.status === "ACTIVE" ? statusSuccess : mutedText }}>● {row.status}</span>
                  </div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: primaryText }}>{row.name}</div>
                  <div style={{ fontSize: 12, color: secondaryText }}>Contact: {row.contact} · {row.phone}</div>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: mutedText, paddingTop: 4, borderTop: `1px solid ${divider}` }}>
                    <span>Materials: <strong style={{ color: secondaryGold }}>{row.materialsCount}</strong></span>
                    <span>Vehicles: <strong style={{ color: primaryText }}>{row.vehiclesCount}</strong></span>
                  </div>
                </div>
              ))}
            </div>
