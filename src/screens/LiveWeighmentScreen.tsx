...
type WeightState = "no-vehicle" | "detecting" | "stabilizing" | "stable" | "capturing" | "captured" | "indicator-error";
type SystemHardwareState = "all-online" | "wb-offline" | "printer-offline" | "camera-offline" | "show-override" | "show-cancel";
...
export default function LiveWeighmentScreen({ darkMode, onToggleDark, onLogout, onNavigate }: Props) {
  ...
  const handleConfirmWeight = () => {
    ...
    setTimeout(() => {
      ...
      alert("Audit Log Event Created: First Weight Captured (24,850 KG) for WB-2026-00463 by Arun Kumar.");
    }, 1200);
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Live Weight Presets Tester */}
        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
          <span style={{ fontSize: 10.5, color: "#94A3B8", fontWeight: 600, textTransform: "uppercase" }}>Weight Preset:</span>
          {[
          ...
          >
            ...
            <div style={{ padding: "14px 20px", borderBottom: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <button onClick={() => onNavigate("vehicle-entry")} style={{ background: "none", border: "none", fontSize: 16, color: primaryOrange, cursor: "pointer" }}>
                  ←
                </button>
                <div>
                  <div style={{ fontSize: 15, fontWeight: 800, color: primaryText }}>First Weighing</div>
                  <div style={{ fontSize: 10.5, color: secondaryGold, fontWeight: 700 }}>WB-01 Main Gate • Step 2</div>
                </div>
              </div>
            </div>
            ...
            <div style={{ padding: "16px 20px 90px 20px", flex: 1, display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <div style={{ padding: 12, borderRadius: 10, background: elevatedSurface, border: `1px solid ${border}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <div>
                  <div style={{ fontSize: 14, fontWeight: 800, color: primaryText, fontFamily: "monospace" }}>TN22GH3456</div>
                  <div style={{ fontSize: 11, color: secondaryText }}>Heavy Truck • Gravel • Sales</div>
                </div>
                <div style={{ fontSize: 10.5, fontWeight: 700, padding: "2px 8px", borderRadius: 4, background: primaryOrangeSoft, color: primaryOrange }}>
                  WB-2026-00463
                </div>
              </div>
              ...
              <div style={{ padding: 20, borderRadius: 16, background: elevatedSurface, border: `2px solid ${currentData.statusColor}`, textAlign: "center" }}>
                <div style={{ fontSize: 11, fontWeight: 800, color: mutedText, letterSpacing: "0.08em", marginBottom: 6 }}>LIVE WEIGHT</div>
                <div style={{ fontSize: 52, fontWeight: 800, fontFamily: "monospace", color: primaryText, letterSpacing: "-0.03em" }}>
                  {currentData.value}
                </div>
                <div style={{ fontSize: 14, fontWeight: 800, color: secondaryGold, marginTop: -4 }}>{currentData.unit}</div>

                <div style={{ marginTop: 12, display: "inline-flex", alignItems: "center", gap: 6, padding: "4px 12px", borderRadius: 999, background: `${currentData.statusColor}20`, color: currentData.statusColor, fontSize: 12, fontWeight: 700 }}>
                  ● {currentData.status}
                </div>
              </div>

              {/* Mobile Device Status Pill Bar */}
              <div style={{ padding: 10, borderRadius: 8, background: elevatedSurface, border: `1px solid ${border}`, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, fontSize: 11 }}>
                <div>Indicator: <strong style={{ color: isIndicatorError ? statusOffline : statusOnline }}>{isIndicatorError ? "✕ Off" : "✓ On"}</strong></div>
                <div>Camera: <strong style={{ color: isCameraOffline ? statusWarning : statusOnline }}>{isCameraOffline ? "✕ Off" : "✓ On"}</strong></div>
              </div>

              {/* Safety Checks */}
              <div style={{ padding: 12, borderRadius: 10, background: elevatedSurface, border: `1px solid ${border}` }}>
                <div style={{ fontSize: 11, fontWeight: 700, color: mutedText, marginBottom: 6 }}>SAFETY CHECKS</div>
            ...
            <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "12px 16px", background: surface, borderTop: `1px solid ${border}`, display: "flex", flexDirection: "column", gap: 8 }}>
              {weightState === "captured" ? (
                ...
                >
                  {isCaptureAllowed ? "CAPTURE FIRST WEIGHT" : "WAITING FOR STABLE WEIGHT..."}
                </button>
              )}
            </div>
          </div>
          ...
          >
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 12, color: mutedText, margin: "2px 0 0 0" }}>
                    Verify scale stability and capture the gross/tare weight for ticket WB-2026-00463.
                  </p>
                </div>
              </div>
            ...
            >
              {[
                ...
                { step: 2, label: "First Weighing", active: true, done: weightState === "captured" },
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    <div
                      style={{
                        ...
                        height: 32,
                        borderRadius: "50%",
                        background: st.done ? statusOnline : st.active ? primaryOrange : dm ? "#374151" : "#E5E7EB",
                    ...
                    <div>
                      ...
                      <div style={{ fontSize: 10.5, color: st.active ? primaryOrange : st.done ? statusOnline : mutedText, fontWeight: 500 }}>
                        {st.done ? "COMPLETED" : st.active ? "ACTIVE WEIGHING" : "PENDING"}
                      </div>
                    </div>
                  </div>
            ...
            {isWbOffline && (
              <div style={{ margin: "16px 32px 0 32px", padding: 14, borderRadius: 8, background: dm ? "#450A0A" : "#FEF2F2", border: `1px solid ${statusOffline}`, color: statusOffline, fontWeight: 700, fontSize: 13 }}>
                ⚠️ WB-01 Weighbridge connection unavailable. Scale capture disabled.
              </div>
            )}

            {isIndicatorError && (
              <div style={{ margin: "16px 32px 0 32px", padding: 14, borderRadius: 8, background: dm ? "#450A0A" : "#FEF2F2", border: `1px solid ${statusOffline}`, color: statusOffline, fontWeight: 700, fontSize: 13 }}>
                🔌 Weight Indicator disconnected. Check physical load cell sensor cable.
              </div>
            )}
            ...
            {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
               ...
               ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
            <div style={{ margin: "16px 32px 0 32px", padding: "14px 20px", borderRadius: 12, background: surface, border: `1px solid ${border}`, display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
                <div>
                  <span style={{ fontSize: 10.5, fontWeight: 700, color: mutedText, letterSpacing: "0.05em" }}>VEHICLE NUMBER</span>
                  <div style={{ fontSize: 18, fontWeight: 800, color: primaryOrange, fontFamily: "monospace", marginTop: 2 }}>
                    TN22GH3456
                  </div>
                </div>
                ...
                <div style={{ fontSize: 12.5, color: secondaryText, display: "flex", gap: 16 }}>
                  <div>Type: <strong style={{ color: primaryText }}>Heavy Truck</strong></div>
                  <div>Driver: <strong style={{ color: primaryText }}>Arun Kumar</strong></div>
                  <div>Customer: <strong style={{ color: primaryText }}>Metro Builders</strong></div>
                  <div>Material: <strong style={{ color: primaryText }}>Gravel</strong></div>
                  <div>Transaction: <strong style={{ color: secondaryGold }}>Sales</strong></div>
                </div>
              </div>

              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: statusInfo, padding: "4px 10px", borderRadius: 6, background: "rgba(37,99,235,0.12)" }}>
                  TICKET # WB-2026-00463
                </span>
                <button
                  onClick={() => onNavigate("vehicle-entry")}
                  style={{ padding: "6px 12px", borderRadius: 6, border: `1px solid ${border}`, background: inputBg, color: secondaryText, fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                >
                  Edit Entry Details
                </button>
              </div>
            ...
            {/* ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
               ...
               ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━ */}
            <div style={{ flex: 1, padding: "20px 32px 32px 32px", display: "grid", gridTemplateColumns: viewDevice === "tablet" ? "1fr" : "1.3fr 0.7fr", gap: 24, overflowY: "auto" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                >
                  ...
                  <div style={{ margin: "16px 0 8px 0", display: "flex", alignItems: "baseline", gap: 16 }}>
                    <div
                      style={{
                        fontSize: 84,
                        fontWeight: 800,
                    ...
                    <div style={{ fontSize: 28, fontWeight: 800, color: secondaryGold, letterSpacing: "0.05em" }}>
                      {currentData.unit}
                    </div>
                  </div>

                  <p style={{ fontSize: 14, color: secondaryText, margin: "8px 0 20px 0", fontWeight: 500 }}>
                    {currentData.message}
                  </p>

                  {/* Scale Status Details Line */}
                  <div style={{ display: "flex", alignItems: "center", gap: 20, fontSize: 12.5, color: mutedText, borderTop: `1px solid ${border}`, paddingTop: 16, width: "100%", justifyContent: "center" }}>
                    <span>Indicator Status: <strong style={{ color: isIndicatorError ? statusOffline : statusOnline }}>{isIndicatorError ? "✕ Disconnected" : "✓ Connected (Loadcell OK)"}</strong></span>
                    <span>Max Scale Capacity: <strong style={{ color: primaryText }}>60 TON</strong></span>
                  </div>

                  {/* Primary Weight Action Button */}
                  <div style={{ marginTop: 24, width: "100%", maxWidth: 440 }}>
                    {weightState === "captured" ? (
                      <div style={{ padding: 16, borderRadius: 12, background: "rgba(22,163,74,0.12)", border: `1.5px solid ${statusOnline}`, textAlign: "center" }}>
                        <div style={{ fontSize: 16, fontWeight: 800, color: statusOnline }}>
                ...
                <div style={{ padding: 20, borderRadius: 14, background: surface, border: `1px solid ${border}` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    ...
                    <span style={{ fontSize: 11, color: statusOnline, fontWeight: 700 }}>● 10Hz SENSOR STREAM</span>
                  </div>

                  <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10 }}>
                    {[
                      { time: "10:55:18", weight: "24,820 KG", status: "Sampling" },
                      { time: "10:55:19", weight: "24,840 KG", status: "Sampling" },
                      { time: "10:55:20", weight: "24,850 KG", status: "Settling" },
                      { time: "10:55:21", weight: "24,850 KG", status: "Settling" },
                      { time: "10:55:22", weight: "24,850 KG", status: "STABLE ✓" },
                    ].map((log, i) => (
                      <div
                        key={log.time}
                        style={{
                          padding: 10,
                          borderRadius: 8,
                          background: i === 4 ? "rgba(22,163,74,0.12)" : elevatedSurface,
                          border: `1px solid ${i === 4 ? statusOnline : border}`,
                          textAlign: "center",
                        }}
                      >
                        <div style={{ fontSize: 10.5, color: mutedText, fontFamily: "monospace" }}>{log.time}</div>
                        <div style={{ fontSize: 13, fontWeight: 800, color: i === 4 ? statusOnline : primaryText, fontFamily: "monospace", marginTop: 2 }}>
                          {log.weight}
                        </div>
                        <div style={{ fontSize: 10, fontWeight: 700, color: i === 4 ? statusOnline : secondaryGold, marginTop: 2 }}>
                          {log.status}
                        </div>
                      </div>
                    ))}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                ...
                <div style={{ padding: 20, borderRadius: 14, background: surface, border: `1px solid ${border}` }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, fontSize: 12.5 }}>
                    {[
                      { label: "Vehicle detected on deck platform", ok: weightState !== "no-vehicle" },
                      { label: "Weight Indicator signal connected", ok: !isIndicatorError },
                      { label: "Scale weight reading stable", ok: weightState === "stable" || weightState === "captured" },
                      { label: "WB-01 Station online & responsive", ok: !isWbOffline },
                      { label: "Operator Arun Kumar authenticated", ok: true },
                      { label: "ANPR Camera feed verified", ok: !isCameraOffline },
                    ].map((chk) => (
                      <div
                        key={chk.label}
                        style={{
                ...
                <div style={{ padding: 20, borderRadius: 14, background: surface, border: `1px solid ${border}` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
                    <h3 style={{ fontSize: 15, fontWeight: 800, margin: 0, color: primaryText }}>Vehicle Camera Feed</h3>
                  ...
                  >
                    <svg width="100%" height="100%" viewBox="0 0 300 140" style={{ opacity: 0.8 }}>
                      <rect x="40" y="30" width="180" height="70" rx="6" fill="#334155" />
                      <path d="M 220 50 L 260 50 L 270 100 L 220 100 Z" fill="#475569" />
                      <circle cx="80" cy="106" r="12" fill="#0F172A" stroke="#64748B" strokeWidth="3" />
                      <circle cx="240" cy="106" r="12" fill="#0F172A" stroke="#F97316" strokeWidth="3" />
                    </svg>
                  ...
                  <div style={{ marginTop: 10, fontSize: 11.5, color: statusOnline, fontWeight: 700 }}>
                    ✓ Vehicle entry image captured & attached to ticket WB-2026-00463
                  </div>
                </div>
              ...
              >
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
                    <div style={{ fontSize: 18, fontWeight: 800, color: primaryText }}>Confirm First Weight</div>
                  ...
                  <div style={{ padding: 16, borderRadius: 12, background: elevatedSurface, border: `1.5px solid ${statusOnline}`, marginBottom: 16, textAlign: "center" }}>
                    <div style={{ fontSize: 11, fontWeight: 800, color: mutedText, letterSpacing: "0.08em" }}>STABLE WEIGHT TO RECORD</div>
                    <div style={{ fontSize: 40, fontWeight: 800, fontFamily: "monospace", color: primaryText, margin: "4px 0" }}>
                      24,850 KG
                    </div>
                    <div style={{ fontSize: 12, color: statusOnline, fontWeight: 700 }}>● STABLE & VERIFIED</div>
                  </div>

                  <div style={{ fontSize: 12.5, color: secondaryText, marginBottom: 20, lineHeight: 1.5 }}>
                    Confirm that vehicle <strong>TN22GH3456</strong> is correctly positioned on WB-01 Main Gate deck.
                  </div>
              ...
              >
                ...
                >
                  ...
                  <p style={{ fontSize: 13, color: secondaryText, margin: "0 0 16px 0", lineHeight: 1.5 }}>
                    This vehicle (TN22GH3456) will not continue through the current weighing process. Select cancellation reason for audit logging.
                  </p>
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 20 }}>
                    ...
                    <div>
                      ...
                      <input type="text" value={overrideReason} onChange={(e) => setOverrideReason(e.target.value)} placeholder="State reason for manual weight override..." required style={{ width: "100%", height: 44, padding: "0 12px", borderRadius: 8, border: `1px solid ${border}`, background: inputBg, color: primaryText, fontSize: 14 }} />
                    </div>
                  </div>
