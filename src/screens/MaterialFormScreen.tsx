...
export default function MaterialFormScreen({
  ...
  const [materialName, setMaterialName] = useState(
    mode === "edit" ? "Blue Metal 20mm" : "",
  );
  const [materialCode, setMaterialCode] = useState(
    mode === "edit" ? "MAT-00041" : "MAT-00047",
  );
  const [category, setCategory] = useState(
    mode === "edit" ? "Aggregates" : "Aggregates",
  );
  const [unit, setUnit] = useState(mode === "edit" ? "TON" : "TON");
  const [status, setStatus] = useState<"ACTIVE" | "INACTIVE">("ACTIVE");
  const [description, setDescription] = useState(
    mode === "edit"
      ? "Standard 20mm crushed granite blue metal aggregate."
      : "",
  );
        ...
        >
          <div>
            ...
            <p style={{ fontSize: 14, color: mutedText, margin: "2px 0 0 0" }}>
              Configure material specifications, unit settings and weighbridge
              assignments.
            </p>
          </div>
